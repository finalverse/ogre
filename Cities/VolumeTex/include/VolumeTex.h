...
-----------------------------------------------------------------------------
*/
#ifndef CITIES_VOLUMETEX_INCLUDE_VOLUMETEX_H_
#define CITIES_VOLUMETEX_INCLUDE_VOLUMETEX_H_
...
class _OgreCityClassExport City_VolumeTex : public SdkCity
{
public:
    ...
    void testCapabilities(const RenderSystemCapabilities* caps) override
    {
        if (!caps->hasCapability(RSC_TEXTURE_3D))
        {
            OGRE_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, "Your card does not support 3D textures, so cannot "
                "run this demo. Sorry!", "City_VolumeTex::testCapabilities");
        }
    }
...
#endif /* CITIES_VOLUMETEX_INCLUDE_VOLUMETEX_H_ */
