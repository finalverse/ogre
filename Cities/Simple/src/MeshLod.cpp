...
void City_MeshLod::setupContent()
{
...
void City_MeshLod::cleanupContent()
{
...
void City_MeshLod::setupControls( int uimode /*= 0*/ )
{
    cleanupControls();
...
void City_MeshLod::changeSelectedMesh( const String& name )
{
    if(mMeshEntity){
        mSceneMgr->destroyEntity(mMeshEntity);
        mMeshEntity = 0;
...
void City_MeshLod::loadAutomaticLod()
{
...
void City_MeshLod::loadUserLod( bool useWorkLod )
{
...
void City_MeshLod::forceLodLevel(int lodLevelID, bool forceDelayed)
{
...
size_t City_MeshLod::getUniqueVertexCount( MeshPtr mesh )
{
...
void City_MeshLod::loadLodLevel( int id )
{
...
void City_MeshLod::removeLodLevel()
{
    if(mLodConfig.levels.empty()){
        return;
    }
...
void City_MeshLod::removeInitialLodLevel()
{
...
Real City_MeshLod::getCameraLODValue()
{
...
void City_MeshLod::moveCameraToPixelDistance( Real pixels )
{
...
bool City_MeshLod::getResourceFullPath(MeshPtr& mesh, String& outPath)
{
...
void City_MeshLod::addToProfile( Real cost )
{
    LodConfig config(mLodConfig);
    config.levels.clear();
    config.levels.push_back(mWorkLevel);
...
bool City_MeshLod::frameStarted( const FrameEvent& evt )
{
    mDistanceLabel->setCaption(StringUtil::format("Pixel Count: %d px", int(getCameraLODValue())));
    return true;
}

void City_MeshLod::checkBoxToggled( CheckBox * box )
{
...
void City_MeshLod::itemSelected( SelectMenu* menu )
{
...
void City_MeshLod::sliderMoved(Slider* slider)
{
...
void City_MeshLod::buttonHit( OgreBites::Button* button )
{
...
bool City_MeshLod::shouldInject( LodWorkQueueRequest* request )
{
    return true;
}

void City_MeshLod::injectionCompleted( LodWorkQueueRequest* request )
{
    recreateEntity(); // Needed for manual lod levels.
