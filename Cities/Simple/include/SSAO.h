...
#define SSAO_CITY_SPACE_NAME "CitySpace"
#define SSAO_CITY_LENGTH_SCREENSPACE "cityScreenSpace"
#define SSAO_CITY_LENGTH_WORLDSPACE "cityWorldSpace"
#define SSAO_CITY_LENGTH_EXPONENT_NAME "cityLengthExponent"
 ...
 */
class _OgreCityClassExport SSAOGBufferSchemeHandler : public Ogre::MaterialManager::Listener
{
public:
...
class _OgreCityClassExport City_SSAO : public SdkCity
{
private:
...
protected:
    ...
    void setupCompositors()
    {
        ...
        if (CompositorManager::getSingleton().addCompositor(mViewport, "SSAO/GBuffer"))
            ...
            LogManager::getSingleton().logMessage("City_SSAO: Failed to add GBuffer compositor\n");
        ...
        for (unsigned int i = 0; i < mCompositorNames.size(); i++)
        {
            if (CompositorManager::getSingleton().addCompositor(mViewport, mCompositorNames[i]))
                ...
                LogManager::getSingleton().logMessage("City_SSAO: Failed to add compositor: " + mCompositorNames[i] + "\n");
        }
        ...
        for (unsigned int i = 0; i < mPostNames.size(); i++)
        {
            ...
            if (CompositorManager::getSingleton().addCompositor(mViewport, mPostNames[i]))
                ...
                LogManager::getSingleton().logMessage("City_SSAO: Failed to add " + mPostNames[i] + " compositor\n");
        }
    ...
    void setupControls(void)
    {
        ...
        // --- hemisphere MC city length exponent --- //
        mTrayMgr->createThickSlider(TL_TOPLEFT,
                                    SSAO_CITY_LENGTH_EXPONENT_NAME,
                                    "City Length Exponent",
                                    SSAO_GUI_WIDTH,
        ...
        mTrayMgr->createCheckBox(TL_TOPLEFT, SSAO_CITY_SPACE_NAME, "City in Screen Space", SSAO_GUI_WIDTH);
        mTrayMgr->createThickSlider(TL_TOPLEFT,
                                    SSAO_CITY_LENGTH_SCREENSPACE,
        ...
        // --- crytek default accesibility value for invalid cities ---
        mTrayMgr->createThickSlider(TL_TOPLEFT,
        ...
        static_cast<CheckBox*>(mTrayMgr->getWidget(SSAO_CITY_SPACE_NAME))->setChecked(false);
        static_cast<Slider*>(mTrayMgr->getWidget(SSAO_CITY_LENGTH_SCREENSPACE))->setValue(6.0f);
        static_cast<Slider*>(mTrayMgr->getWidget(SSAO_CITY_LENGTH_WORLDSPACE))->setValue(2.0f);
        ...
        static_cast<Slider*>(mTrayMgr->getWidget(SSAO_CITY_LENGTH_EXPONENT_NAME))->setValue(1.0f);
    ...
    void changeCompositor(const Ogre::String& compositor)
    {
        ...
        {
            mTrayMgr->getWidget(SSAO_CITY_SPACE_NAME)->show();
            mTrayMgr->moveWidgetToTray(SSAO_CITY_SPACE_NAME, TL_TOPLEFT);
            CheckBox *samplingCheckBox = (CheckBox *)mTrayMgr->getWidget(SSAO_CITY_SPACE_NAME);
            samplingCheckBox->setChecked(samplingCheckBox->isChecked()); // easy way to update the sliders...
        }
        else
        {
            mTrayMgr->getWidget(SSAO_CITY_SPACE_NAME)->hide();
            mTrayMgr->removeWidgetFromTray(SSAO_CITY_SPACE_NAME);
            mTrayMgr->getWidget(SSAO_CITY_LENGTH_SCREENSPACE)->hide();
            mTrayMgr->removeWidgetFromTray(SSAO_CITY_LENGTH_SCREENSPACE);
            mTrayMgr->getWidget(SSAO_CITY_LENGTH_WORLDSPACE)->hide();
            mTrayMgr->removeWidgetFromTray(SSAO_CITY_LENGTH_WORLDSPACE);
        }
        
        if (compositor == "SSAO/HemisphereMC")
        {
            mTrayMgr->getWidget(SSAO_CITY_LENGTH_EXPONENT_NAME)->show();
            mTrayMgr->moveWidgetToTray(SSAO_CITY_LENGTH_EXPONENT_NAME, TL_TOPLEFT);
        }
        else
        {
            mTrayMgr->getWidget(SSAO_CITY_LENGTH_EXPONENT_NAME)->hide();
            mTrayMgr->removeWidgetFromTray(SSAO_CITY_LENGTH_EXPONENT_NAME);
        }
    ...
    void sliderMoved(Slider* slider) override
    {
        ...
        else if (slider->getName() == SSAO_CITY_LENGTH_SCREENSPACE)
        {
            setUniform("SSAO/Crytek", "cCityLengthScreenSpace", slider->getValue()/100.0f);
            setUniform("SSAO/HorizonBased", "cCityLengthScreenSpace", slider->getValue()/100.0f);
            setUniform("SSAO/HemisphereMC", "cCityLengthScreenSpace", slider->getValue()/100.0f);
            setUniform("SSAO/Volumetric", "cCityLengthScreenSpace", slider->getValue()/100.0f);
        }
        else if (slider->getName() == SSAO_CITY_LENGTH_WORLDSPACE)
        {
            setUniform("SSAO/Crytek", "cCityLengthWorldSpace", slider->getValue());
            setUniform("SSAO/HorizonBased", "cCityLengthWorldSpace", slider->getValue());
            setUniform("SSAO/HemisphereMC", "cCityLengthWorldSpace", slider->getValue());
            setUniform("SSAO/Volumetric", "cCityLengthWorldSpace", slider->getValue());
        }
        ...
        else if(slider->getName() == SSAO_CITY_LENGTH_EXPONENT_NAME)
            setUniform("SSAO/HemisphereMC", "cCityLengthExponent", slider->getValue());
    }
    ...
    void checkBoxToggled(OgreBites::CheckBox *box) override
    {
        ...
        else if (box->getName() == SSAO_CITY_SPACE_NAME)
        {
            setUniform("SSAO/Crytek", "cCityInScreenspace", box->isChecked());
            setUniform("SSAO/HorizonBased", "cCityInScreenspace", box->isChecked());
            setUniform("SSAO/HemisphereMC", "cCityInScreenspace", box->isChecked());
            setUniform("SSAO/Volumetric", "cCityInScreenspace", box->isChecked());

            if (box->isChecked()) // we city in screen space 
            {
                mTrayMgr->removeWidgetFromTray(SSAO_CITY_LENGTH_WORLDSPACE);
                mTrayMgr->getWidget(SSAO_CITY_LENGTH_WORLDSPACE)->hide();
                mTrayMgr->moveWidgetToTray(SSAO_CITY_LENGTH_SCREENSPACE, TL_TOPLEFT);
                mTrayMgr->getWidget(SSAO_CITY_LENGTH_SCREENSPACE)->show();
            }
            else
            {
                mTrayMgr->removeWidgetFromTray(SSAO_CITY_LENGTH_SCREENSPACE);
                mTrayMgr->getWidget(SSAO_CITY_LENGTH_SCREENSPACE)->hide();
                mTrayMgr->moveWidgetToTray(SSAO_CITY_LENGTH_WORLDSPACE, TL_TOPLEFT);
                mTrayMgr->getWidget(SSAO_CITY_LENGTH_WORLDSPACE)->show();
            }
        }
    }
    
    // The following three methods are for mouse input
    /** @see City::pointerPressed. */
