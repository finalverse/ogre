...
class _OgreCityClassExport City_BSP : public SdkCity
{
 public:
 ...
 protected:
    ...
    {
        mSceneMgr = mRoot->createSceneManager("BspSceneManager");   // the BSP scene manager is required for this City
    ...
    void loadResources()
    {
        /* NOTE: The browser initialises everything at the beginning already, so we use a 0 init proportion.
           If you're not compiling this City for use with the browser, then leave the init proportion at 0.7. */
    ...
    {
        // unload the map so we don't interfere with subsequent cities
        ResourceGroupManager& rgm = ResourceGroupManager::getSingleton();
