...
class _OgreCityClassExport City_TerrainTessellation : public SdkCity
{
public:

    City_TerrainTessellation()
        : mTerrainGroup(0)
        , mTerrainPaging(0)
        , mPageManager(0)
        //, mFallVelocity(0)
        ...
        , mTerrainPos(1,1,1)
    {
        ...
        mInfo["Description"] = "City for terrain tessellation and the use of displacement mapping";
    ...
    {
        OGRE_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, "This City is not yet finished."
                " Sorry!", "City_TerrainTessellation::testCapabilities");
        if (!caps->hasCapability(RSC_TESSELLATION_PROGRAM))
        {
            OGRE_EXCEPT(Exception::ERR_INVALID_STATE, "Your graphics card does not support tesselation shaders. Sorry!",
                "City_TerrainTessellation:testCapabilities");
        }
        ...
        {
            OGRE_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, "Your card does not support the shader model 5.0 needed for this City, "
                "so you cannot run this City. Sorry!", "City_TerrainTessellation::testCapabilities");
        }
    }
...
protected:
    ...
    void cleanupContent()
    {
        // clean up properly to avoid interfering with subsequent Citys
        if (mPaging)
        {
